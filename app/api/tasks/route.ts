...
type TaskInput = { repositoryId?: string; title?: string; prompt?: string; actor?: string; model?: string; baseBranch?: string; mode?: string; runtime?: string; steps?: StepInput[] };

const supportedActors = ["pi", "codex", "claude"];
...
export async function POST(request: Request) {
  ...
  if (!steps.length || steps.some((step) => !supportedActors.includes(step.actor))) return Response.json({ error: "Unsupported actor in execution plan" }, { status: 400 });
  ...
  const statements = [
    database().prepare("INSERT INTO tasks (id, owner_id, repository_id, title, prompt, actor, model, mode, runtime, base_branch, status, attempt, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'queued', 1, ?, ?)")
      .bind(id, auth.ownerId, repositoryId, title, prompt, primary.actor, primary.model, mode, runtime, baseBranch, timestamp, timestamp),
  ...
  return Response.json({ task: { id, repositoryId, title, prompt, actor: primary.actor, model: primary.model, mode, runtime, baseBranch, status: "queued", attempt: 1, createdAt: timestamp, updatedAt: timestamp }, sessions: steps.map((step) => ({ ...step, taskId: id, status: "pending", createdAt: timestamp, updatedAt: timestamp })) }, { status: 201 });
}
